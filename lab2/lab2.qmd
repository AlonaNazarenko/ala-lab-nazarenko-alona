...
Thanks to this [site](https://dou.ua/forums/topic/47053/), where I got to know how eigenvectors are related to PCA, and I was also surprised that the covariance matrix’s eigenvectors construct an orthonormal matrix.
...
2. The encryption uses eigenvalue decomposition: **K = PDP⁻¹**
...
**Eigenvalues** (λ) and **eigenvectors** (v) of a square matrix **A** satisfy:
$$A \cdot v = \lambda \cdot v$$
...
2. Find eigenvalues λ
3. For each λ, solve **(A - λI)v = 0** to find eigenvectors
...
- The matrix can be diagonalized as: **A = QΛQᵀ** where Q is orthogonal
...
**Disadvantages:**
1. **Loss of interpretability** — principal components are linear combinations of original features
2. **Assumes linearity** — may not capture complex nonlinear relationships
3. **Sensitive to scaling** — features must be standardized
4. **Variance ≠ importance** — high-variance components aren't always most meaningful
...
- **Decryption:** Uses **K⁻¹ = PD⁻¹P⁻¹** where K = PDP⁻¹
...
1. **Direct computation** verified the fundamental equation A·v = λ·v
